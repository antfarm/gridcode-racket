...
(select x y)
(select x y key)
(select-neighbors x y 'moore)
(select-neighbors x y 'von-neumann)
(select-neighbors x y 'horizontal r)
(select-neighbors x y 'vertical r)
(select-neighbors x y 'moore r)
(select-neighbors x y 'von-neumann r)
(select-neighbors x y 'moore key)
(select-neighbors x y 'horizontal r key)
...
(define (setup-grid)
   ...
   (with (select-row (- rows 1)) as (x y _)
     (set-cell! x y 'out))

   (with (select-neighbors center (- rows 1) 'horizontal 2) as (x y _)
     (set-cell! x y 'paddle))

   (define ball-x (+ (- center 2) (random 5)))
