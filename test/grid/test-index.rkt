...
(test-case "index-set! flag — cell appears in table index"
  (index-clear!)
  (index-set! 3 4 'wall)
  (check-true (set-member? (index-select 'wall) '(3 4))))

(test-case "index-set! flag — only affects named table"
  (index-clear!)
  (index-set! 3 4 'wall)
  (check-false (set-member? (index-select 'ball) '(3 4))))

(test-case "index-set! flag — multiple cells in same table"
  (index-clear!)
  (index-set! 1 1 'wall)
  (index-set! 2 3 'wall)
  (check-equal? (index-select 'wall) (set '(1 1) '(2 3))))

(test-case "index-set! flag — repeated call is idempotent"
  (index-clear!)
  (index-set! 5 5 'wall)
  (index-set! 5 5 'wall)
...
(test-case "index-set! data — cell appears in all three indices"
  (index-clear!)
  (index-set! 1 2 'ball 'dx #f 1)
...
(test-case "index-set! data — different keys are indexed independently"
  (index-clear!)
  (index-set! 5 5 'ball 'dx #f  1)
  (index-set! 5 5 'ball 'dy #f -1)
...
(test-case "index-set! data — updating value removes coord from old value bucket"
  (index-clear!)
  (index-set! 5 5 'ball 'dx #f 1)
  (index-set! 5 5 'ball 'dx 1  2)  ; old-value = 1, new-value = 2
  (check-false (set-member? (index-select 'ball 'dx 1) '(5 5)))
  (check-true  (set-member? (index-select 'ball 'dx 2) '(5 5))))

(test-case "index-set! data — updating value keeps coord in table and key index"
  (index-clear!)
  (index-set! 5 5 'ball 'dx #f 1)
  (index-set! 5 5 'ball 'dx 1  2)
  (check-true (set-member? (index-select 'ball)     '(5 5)))
  (check-true (set-member? (index-select 'ball 'dx) '(5 5))))

(test-case "index-set! data — multiple coords in same value bucket"
  (index-clear!)
  (index-set! 1 1 'enemy 'state #f 'active)
  (index-set! 2 2 'enemy 'state #f 'active)
  (index-set! 3 3 'enemy 'state #f 'inactive)
...
;; ---- index-delete! whole table ----

(test-case "index-delete! table — removes coord from table index"
  (index-clear!)
  (index-set! 5 5 'wall 'strength #f 3)
  (let ([old-table (hash 'strength 3)])
    (index-delete! 5 5 'wall old-table))
  (check-false (set-member? (index-select 'wall) '(5 5))))

(test-case "index-delete! table — removes coord from key index"
  (index-clear!)
  (index-set! 5 5 'wall 'strength #f 3)
  (let ([old-table (hash 'strength 3)])
    (index-delete! 5 5 'wall old-table))
...
(test-case "index-delete! table — removes coord from value index"
  (index-clear!)
  (index-set! 5 5 'wall 'strength #f 3)
  (let ([old-table (hash 'strength 3)])
    (index-delete! 5 5 'wall old-table))
...
(test-case "index-delete! table — flag-only (empty table hash)"
  (index-clear!)
  (index-set! 5 5 'marker)
  (index-delete! 5 5 'marker (make-hash))
  (check-false (set-member? (index-select 'marker) '(5 5))))

(test-case "index-delete! table — other coords in same table are unaffected"
  (index-clear!)
  (index-set! 1 1 'wall 'strength #f 3)
  (index-set! 2 2 'wall 'strength #f 3)
  (index-delete! 1 1 'wall (hash 'strength 3))
  (check-false (set-member? (index-select 'wall) '(1 1)))
  (check-true  (set-member? (index-select 'wall) '(2 2))))

;; ---- index-delete! single key ----

(test-case "index-delete! key — removes coord from key index"
  (index-clear!)
  (index-set! 5 5 'ball 'dx #f 1)
  (index-set! 5 5 'ball 'dy #f -1)
  (index-delete! 5 5 'ball 'dx 1)
  (check-false (set-member? (index-select 'ball 'dx) '(5 5)))
  (check-true  (set-member? (index-select 'ball 'dy) '(5 5))))

(test-case "index-delete! key — removes coord from value index"
  (index-clear!)
  (index-set! 5 5 'ball 'dx #f 1)
  (index-delete! 5 5 'ball 'dx 1)
  (check-false (set-member? (index-select 'ball 'dx 1) '(5 5))))

(test-case "index-delete! key — keeps coord in table index"
  (index-clear!)
  (index-set! 5 5 'ball 'dx #f 1)
  (index-set! 5 5 'ball 'dy #f -1)
  (index-delete! 5 5 'ball 'dx 1)
...
(test-case "index-set-table! — adds coord to all three indices for new keys"
  (index-clear!)
  (index-set-table! 3 3 'ball #f (hash 'dx 1 'dy -1))
...
(test-case "index-set-table! — replaces old table entries"
  (index-clear!)
  (index-set! 3 3 'ball 'dx #f 1)
  (index-set-table! 3 3 'ball (hash 'dx 1) (hash 'dx 2))
  (check-false (set-member? (index-select 'ball 'dx 1) '(3 3)))
  (check-true  (set-member? (index-select 'ball 'dx 2) '(3 3))))

(test-case "index-set-table! — removes old keys not present in new table"
  (index-clear!)
  (index-set! 3 3 'ball 'dx #f 1)
  (index-set! 3 3 'ball 'dy #f -1)
  (index-set-table! 3 3 'ball (hash 'dx 1 'dy -1) (hash 'dx 2))
